...
- **6.25U スペース option（SW25-1）**: 電気的に Fn1 ノード（`4,4`）を共有。
  回路図のキーサイズが `6.25U = Space(2.25) + Fn1(1.25) + Delete(2.75)` と一致する
  ため、6.25U バーは `4,4`（SW25 / SW25-1）に載り、`4,3`（Space）と `4,5`（Delete）
  は非実装になる。これを `keyboard.json` の **`LAYOUT_6_25u`**（58 キー）として定義済み。
  既定は分割スペース版 `LAYOUT`（PRK 由来・出荷時配列）を採用し、`LAYOUT_6_25u` は
  6.25U プレート向けの代替として用意する。

## LAYOUT_6_25u の最下段（`LAYOUT` との差分のみ）

| 物理 | 電気[Row,Col] | サイズ | 既定キーコード |
|---|---|---|---|
| LCtrl | 4,0 | 1.25 | LCtrl |
| LGUI | 4,1 | 1.25 | LGUI |
| LAlt | 4,2 | 1.25 | LAlt |
| Space(6.25U) | 4,4 | 6.25 | `LT(_FN, KC_ENT)`（= 旧 Fn1 ノード） |
| Fn2 | 9,5 | 1 | `LT(_RGB, KC_SPC)` |
| ← ↓ → | 9,4 / 9,3 / 9,2 | 1 | Left / Down / Right |

> 6.25U 版では Fn1 ノード（`4,4`）が唯一の大型キーになる。用途に応じて
> VIA/Vial でこの位置を `KC_SPC` などに割り当て直すこと。VIA/Vial GUI 上での
